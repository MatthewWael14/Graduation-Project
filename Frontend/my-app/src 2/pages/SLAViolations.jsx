...
export default function SLAViolations({ user }) {
  ...
  const handleEscalate = (slaId, target) => {
    setEscalated(prev => ({ ...prev, [slaId]: target }));
  };

  const totalExposure = slaData.filter(s=>s.violationStatus).reduce((acc,s) => acc + (s.penaltyDaily * Math.max(0, s.delayDays - 2)), 0);
    ...
    <div>
      ...
      <div style={S.pageHeader}>
        ...
        <div style={S.pageDesc}>
          {isProcurement
            ? "View SLA contracts, monitor breaches, contact suppliers and escalate to production"
            : "SLA Violation Detection · Financial Penalty Calculation · Compliance Dashboard"}
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:20 }}>
        {[
          ...
          { label:"Total Exposure",   value:"$"+totalExposure.toLocaleString(),                                              color:C.orange },
        ].map((k,i) => (
          ...
          {slaData.map((s,i) => (
            <div key={i} className="data-row"
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                ...
                {/* Escalate — Procurement can escalate to Production, Admin can escalate anywhere */}
                {s.violationStatus && (isProcurement || isAdmin) && (
                  escalated[s.id] ? (
                    <span style={{ ...S.badge(C.green), fontSize:9 }}>✓ Escalated to {escalated[s.id]}</span>
                  ) : (
                    <button style={{ ...S.btn("danger"), fontSize:10, padding:"4px 10px" }}
                      onClick={() => handleEscalate(s.id, isProcurement ? "Production Manager" : "Procurement Manager")}>
                      🚨 Escalate to {isProcurement ? "Production" : "Procurement"}
                    </button>
                  )
                )}
              </div>
